...
//     let recommended = [];
//     if(email){
//       recommended = await axios.get(`${process.env.AI_SERVER}/recommend?email=${email}`)
//     }
...
Videos.get("/getvideos", async (req, res) => {
  try {
    ...
    const allVideos = videos.flatMap(video => video.VideoData);

    // Separate recommended videos
    // const recommendedVideos = allVideos.filter(video => recommendedVideoIds.includes(video._id.toString()));
    const otherVideos = allVideos.filter(video => !recommendedVideoIds.includes(video._id.toString()));
    
    let  recommendedVideos = []
    recommendedVideoIds.forEach((_id) => {
      const video = allVideos.find((video) => video._id.toString() == _id)
      recommendedVideos.push(video)
    })
    recommendedVideos.forEach(video => console.log(video._id))



    // Combine recommended and other videos
    const orderedVideos = [...recommendedVideos, ...otherVideos];

    // Extract information from orderedVideos
    const videoURLs = orderedVideos.map(video => video.videoURL);
    const thumbnailURLs = orderedVideos.map(video => video.thumbnailURL);
    const titles = orderedVideos.map(video => video.Title);
    const Uploader = orderedVideos.map(video => video.uploader);
    const Duration = orderedVideos.map(video => video.videoLength);
    const Profile = orderedVideos.map(video => video.ChannelProfile);
    const videoID = orderedVideos.map(video => video.id);
    const comments = orderedVideos.map(video => video.comments);
    const views = orderedVideos.map(video => video.views);
    const uploadDate = orderedVideos.map(video => video.uploaded_date);
    const Likes = orderedVideos.map(video => video.likes);
    const Visibility = orderedVideos.map(video => video.visibility);
    const isLive = orderedVideos.map(video => video.isLive);
...
// go-live
Videos.post("/go-live", async (req, res) => {
  try {
    const {
      videoTitle,
      videoDescription,
    ...
    const user = await userData.findOne({ email });
    let videos = await videodata.findOne({ email });
    const streamKey = uuid.v4().toString();
    const RTMP_URL = `${process.env.RTMP_SERVER_HOST}/${streamKey}`
    const HLS_URL = `${process.env.HLS_SERVER_HOST}/${streamKey}/index.m3u8`
    ...
    if (user) {
      ...
      return res.status(200).json({success: true,RTMP_URL: process.env.RTMP_SERVER_HOST,streamKey});
    } else {
      return res.status(404).json({ message: "User not found" });
    }
  } catch (error) {
