...
Studio.post("/stoplivestream/:streamKey", async (req, res) => {
  try {
    const { streamKey } = req.params;
    const { videoURL } = req.body;

    let video = await videodata.findOne({ "VideoData.streamKey": streamKey })
    video = video.VideoData.find(v => v.streamKey == streamKey)
    if(!video){
      res.status(404).json({
        success: false,
    ...
    await userData.updateMany(
      ...
      {
        $set: {
          "likedVideos.$.videoURL": videoURL,
          "likedVideos.$.isLive": false,
        },
      }
    );
    ...
    await userData.updateMany(
      ...
      {
        $set: {
          "watchLater.$.videoURL": videoURL,
          "watchLater.$.isLive": false,
        },
      }
    );
    ...
    await userData.updateMany(
      ...
      {
        $set: {
          "Playlists.$[playlist].playlist_videos.$[video].videoURL": videoURL,
          "Playlists.$[playlist].playlist_videos.$[video].isLive": false,
        },
      },
      {
        arrayFilters: [
